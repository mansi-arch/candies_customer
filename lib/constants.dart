...
import 'package:foodie_customer/model/admin_commission.dart';
...
bool specialDiscountOffer = false;
...
String productCommissionPrice(String Price) {
  String commission = "0";
  if(adminCommissionModel != null && adminCommissionModel!.isEnabled == true){
    if (adminCommissionModel!.adminCommissionType!.toLowerCase() == "Percent".toLowerCase() || adminCommissionModel!.adminCommissionType?.toLowerCase() == "Percentage".toLowerCase()) {
      commission = (double.parse(Price) + (double.parse(Price) * double.parse(adminCommissionModel!.adminCommission.toString()) / 100)).toString();
    } else {
      commission = (double.parse(Price) + double.parse(adminCommissionModel!.adminCommission.toString())).toString();
    }
  }else{
    commission = Price;
  }
