...
import 'package:foodie_customer/model/zone_model.dart';
...
class FireStoreUtils {
  ...
  Future<List<VendorCategoryModel>> getHomePageShowCategory() async {
    ...
    QuerySnapshot<Map<String, dynamic>> cuisinesQuery =
        await firestore.collection(VENDORS_CATEGORIES).where("show_in_homepage", isEqualTo: true).where('publish', isEqualTo: true).get();
  ...
  Future<Url> uploadChatImageToFireStorage(File image, BuildContext context) async {
    showProgress('Uploading image...', false);
    var uniqueID = Uuid().v4();
  ...
  Future<ChatVideoContainer> uploadChatVideoToFireStorage(File video, BuildContext context) async {
    showProgress('Uploading video...', false);
    var uniqueID = Uuid().v4();
  ...
  Future<Url> uploadAudioFile(File file, BuildContext context) async {
    showProgress('Uploading Audio...', false);
    var uniqueID = Uuid().v4();
  ...
  Future<List<VendorModel>> getVendors() async {
    List<VendorModel> vendors = [];
    QuerySnapshot<Map<String, dynamic>> vendorsQuery = await firestore.collection(VENDORS).where("zoneId", isEqualTo: selectedZone!.id.toString()).get();
  ...
  Stream<List<VendorModel>> getAllRestaurants({String? path}) async* {
    ...
    try {
      var collectionReference = (path == null || path.isEmpty)
          ? firestore.collection(VENDORS).where("zoneId", isEqualTo: selectedZone!.id.toString())
          : firestore.collection(VENDORS).where("zoneId", isEqualTo: selectedZone!.id.toString()).where("enabledDiveInFuture", isEqualTo: true);
  ...
  Stream<List<VendorModel>> getCategoryRestaurants(String categoryId) async* {
    ...
    try {
      var collectionReference = firestore.collection(VENDORS).where("zoneId", isEqualTo: selectedZone!.id.toString()).where('categoryID', isEqualTo: categoryId);
  ...
  Stream<List<VendorModel>> getVendorsByCuisineID(String cuisineID, {bool? isDinein}) async* {
    ...
    var collectionReference = isDinein!
        ? firestore
            .collection(VENDORS)
            .where("zoneId", isEqualTo: selectedZone!.id.toString())
            .where('categoryID', isEqualTo: cuisineID)
            .where("enabledDiveInFuture", isEqualTo: true)
        : firestore.collection(VENDORS).where("zoneId", isEqualTo: selectedZone!.id.toString()).where('categoryID', isEqualTo: cuisineID);
    GeoFirePoint center = geo.point(latitude: MyAppState.selectedPosotion.location!.latitude, longitude: MyAppState.selectedPosotion.location!.longitude);
    String field = 'g';
    Stream<List<DocumentSnapshot>> stream = geo.collection(collectionRef: collectionReference).within(center: center, radius: radiusValue, field: field, strictMode: true);
    stream.listen((List<DocumentSnapshot> documentList) {
  ...
  Future<List<OfferModel>> getAllCoupons() async {
    ...
    QuerySnapshot<Map<String, dynamic>> couponsQuery =
        await firestore.collection(COUPON).where('expiresAt', isGreaterThanOrEqualTo: Timestamp.now()).where("isEnabled", isEqualTo: true).get();
    await Future.forEach(couponsQuery.docs, (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
        coupon.add(OfferModel.fromJson(document.data()));
      } catch (e) {
  ...
  Future<VendorCategoryModel?> getVendorCategoryById(String vendorCategoryID) async {
    ...
    QuerySnapshot<Map<String, dynamic>> vendorsQuery =
        await firestore.collection(VENDORS_CATEGORIES).where('id', isEqualTo: vendorCategoryID).where('publish', isEqualTo: true).get();
    try {
      print('we are enter-->');
  ...
  static firebaseSignUpWithEmailAndPassword(
      String emailAddress, String password, File? image, String firstName, String lastName, String mobile, BuildContext context, String referralCode) async {
    try {
  ...
  static Future<List<ZoneModel>?> getZone() async {
    ...
    await firestore.collection(ZONE).where('publish', isEqualTo: true).get().then((value) {
      for (var element in value.docs) {
