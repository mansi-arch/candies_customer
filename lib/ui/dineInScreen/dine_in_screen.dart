...
import 'package:foodie_customer/ui/zone_not_avavilbale.dart';
    ...
    await FireStoreUtils.getZone().then((value) {
      if (value != null) {
        for (int i = 0; i < value.length; i++) {
          if (isPointInPolygon(LatLng(MyAppState.selectedPosotion.location!.latitude, MyAppState.selectedPosotion.location!.longitude), value[i].area!)) {
    ...
    if (isZoneAvailable == false) {
      pushAndRemoveUntil(
          ...
          ContainerScreen(
            ...
            currentWidget: ZoneNotAvailable(),
            appBarTitle: 'Dine-In'.tr(),
            drawerSelection: DrawerSelection.dineIn,
          ),
          false);
    }
  }
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Text(MyAppState.selectedPosotion.getFullAddress().toString(),
                                      maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: Color(COLOR_PRIMARY), fontFamily: "Poppinsr"))
                                  .tr(),
                            ),
                            ElevatedButton(
                                onPressed: () async {
                                  if (MyAppState.currentUser != null) {
                                    ...
                                    checkPermission(
                                          () async {
                                        await showProgress("Please wait...".tr(), true);
                                        AddressModel addressModel = AddressModel();
                                        try {
                                          await Geolocator.requestPermission();
                                          await Geolocator.getCurrentPosition();
                      ...
                      Padding(
                        ...
                        child: Text("Find your restaurant", style: TextStyle(fontSize: 24, color: isDarkMode(context) ? Colors.white : Color(0xFF333333), fontFamily: "Poppinssb"))
                            .tr(),
                      ),
                      buildDineInTitleRow(
