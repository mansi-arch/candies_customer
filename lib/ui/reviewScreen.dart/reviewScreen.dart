...
class _ReviewScreenState extends State<ReviewScreen> with TickerProviderStateMixin {
                                      ...
                                      child: Column(children: [
                                        ...
                                        Container(
                                            ...
                                            child: Text(widget.product.name,
                                                style: TextStyle(
                                                    color: isDarkMode(context) ? const Color(0XFFFDFEFE) : const Color(0XFF000003), fontFamily: 'Poppinsm', fontSize: 20))),
                                        const SizedBox(
                                          height: 15,
                                        ),
                                        RatingBar.builder(
          ...
          bottomNavigationBar: ratingModel != null
              ? Padding(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      await showProgress( 'Updating data to database...'.tr(), false);
  ...
  savereview() async {
    if (comment.text == '' || ratings == 0) {
      ...
      await showProgress( 'Saving data to database...'.tr(), false);
      List<String> mediaFilesURLs = _mediaFiles.whereType<String>().toList().cast<String>();
      List<File> imagesToUpload = _mediaFiles.whereType<File>().toList().cast<File>();
      if (imagesToUpload.isNotEmpty) {
        for (int i = 0; i < imagesToUpload.length; i++) {
